...
export class CartService {
  ...
  getCart(): Observable<Cart> {
    return this.api.get<Cart>(this.getCartEndpoint()).pipe(
      map(r => this.normalizeCart(r.data)),
      tap(cart => {
  ...
  private loadCart(): void {
    this.api.get<Cart>(this.getCartEndpoint()).pipe(map(r => this.normalizeCart(r.data))).subscribe({
      next: cart => {
        cart.total = cart.items.reduce((sum, i) => sum + i.totalPrice, 0);
        cart.itemCount = cart.items.reduce((sum, i) => sum + i.quantity, 0);
        this.cartSubject.next(cart);
      },
      error: () => this.cartSubject.next(this.cartSubject.value)
    });
  }

  private normalizeCart(data: Cart | null | undefined): Cart {
    const empty: Cart = { id: '', items: [], total: 0, itemCount: 0, createdAt: new Date().toISOString() };
